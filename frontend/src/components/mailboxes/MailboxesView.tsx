...
export const MailboxesView: React.FC = () => {
  ...
  const handleSendEmail = (e: React.FormEvent) => {
    ...
    if (currentMailbox?.id && /^\d+$/.test(currentMailbox.id)) {
  ...
  const handleCreateMailbox = (e: React.FormEvent) => {
    ...
    if (selDomain?.id && /^\d+$/.test(selDomain.id) && newPassword) {
      void api.createMailbox({ domainId: selDomain.id, localPart: newEmailPrefix.trim().toLowerCase(), password: newPassword }).catch(() => undefined);
    }
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D0D0D] border border-white/15 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in">
            ...
            <form onSubmit={handleCreateMailbox} className="p-6 space-y-4 text-xs">
              ...
              <div>
                <label className="block text-white/50 mb-1 font-mono uppercase text-[10px]">Mailbox Password</label>
                <input
                  type="password"
                  required
                  minLength={8}
