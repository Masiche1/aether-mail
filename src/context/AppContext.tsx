...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Autonomous Queue Simulation Loop
  useEffect(() => {
    if (!isEngineRunning) return;

    const interval = setInterval(() => {
      setQueueJobs((prevJobs) => {
        const hasSendingOrQueued = prevJobs.some((j) => j.status === 'queued' || j.status === 'sending');
        if (!hasSendingOrQueued) return prevJobs;
            ...
            return {
              ...
              events: [
                ...job.events,
                {
                  ...
                  event: 'DISPATCHED',
                  details: 'Allocated to MTA worker thread on mail01.aethermail.net',
                },
              ],
            };
          }
          if (job.status === 'sending') {
            ...
            if (isSuccess) {
              return {
                ...
                status: 'delivered',
                deliveredTime: now,
                smtpResponse: '250 2.0.0 OK (autonomous delivery confirmed)',
                events: [
                  ...job.events,
                  {
                    timestamp: now,
                    event: 'DELIVERED',
                    details: 'Accepted by remote recipient MX mail server (250 OK)',
                  },
                ],
              };
            } else {
              return {
                ...
                lastError: '451 4.4.0 Connection timed out; scheduled backoff attempt.',
                events: [
                  ...job.events,
                  {
                    timestamp: now,
                    event: 'TEMPORARY_FAILURE',
                    details: 'Remote MX busy. Smart Retry Scheduler scheduled exponential backoff.',
                  },
                ],
              };
            }
